  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
        ...
        <div className="rounded-xl border border-indigo-200 dark:border-indigo-500/20 bg-indigo-50/20 dark:bg-gradient-to-br dark:from-[#0f1117] dark:to-indigo-950/20 p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-indigo-500/15">
                <FileText className="h-3.5 w-3.5 text-indigo-600 dark:text-indigo-400" />
              </div>
              <h3 className="text-sm font-bold uppercase tracking-wider text-slate-900 dark:text-white">Ekstraksi Mutasi</h3>
            </div>
            <span className="rounded-full bg-emerald-500/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-emerald-600 dark:text-emerald-400">Aktif</span>
          </div>
          <p className="text-[13px] text-slate-500 dark:text-slate-400 mb-4">Import otomatis dari PDF mutasi rekening bank</p>
          ...
          <div className="mb-3 rounded-lg border border-emerald-500/25 dark:border-emerald-500/20 bg-emerald-500/10 dark:bg-emerald-500/5 px-3 py-2">
            <p className="text-[11px] text-emerald-700 dark:text-emerald-400">🔒 File tidak disimpan — diproses di memory server saja</p>
          </div>
          <button
            type="button"
            onClick={() => setShowMutasiModal(true)}
      ...
      {data.length ? (
        <div className="grid gap-4 md:grid-cols-[220px_1fr]">
          ...
          <div className="space-y-2 self-center">
            {data.map((item, index) => (
              <div key={item.name} className="flex items-center justify-between gap-3 rounded-lg bg-slate-50 dark:bg-[#0f1117]/70 px-3 py-2 border border-slate-100 dark:border-none">
                ...
                <div className="text-right">
                  <p className={cn("text-sm font-bold", tone === "income" ? "text-emerald-600 dark:text-emerald-300" : "text-rose-600 dark:text-rose-300")}>{item.percentage.toFixed(1)}%</p>
                  <p className="text-xs text-slate-500 dark:text-muted-foreground">{formatCompact(item.amount)}</p>
                </div>
              </div>
            ))}
          </div>
